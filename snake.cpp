...
bool eventTriggered(double interval) {
    ...
    if (currentTime - lastUpdateTime >= interval) {
        ...
        return true;   // returns true after a certain interval has passed.
    }
    return false;
}
...
public:
    ...
    void Update() {
        body.push_front(Vector2Add(body[0], direction)); // moves the snake by continuously pushing a new vector and popping the last vector till it eats the food.
        if (addSegment == true) {
            addSegment = false;
        } else {
            body.pop_back();
...
public:
    ...
    Food(deque<Vector2> snakeBody) {
        ...
        Image image = LoadImage("./image/food.png"); // leads the image from the file to the structure Image.
...
public:
    ...
    ~Game() {
        SaveHighScore(); // Save high score when the game ends
    }
    ...
    void CheckCollisionWithEdges() {
        if (snake.body[0].x == cellCountX || snake.body[0].x == -1 ||
            snake.body[0].y == cellCountY || snake.body[0].y == -1) {
            GameOver();
        }
    }
    ...
    void DrawScore() {
        ...
        DrawText(TextFormat("High Score: %04i", highScore), 10, 30, 20, BLACK);
        if (gameOver) {
            DrawText("Game Over! Press Q to quit", 10, 50, 20, RED);
        }
    }
...
int snake() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (game.gameOver && IsKeyPressed(KEY_Q)) {
            CloseWindow();
            return 0;
        }
