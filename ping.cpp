 ...
 void Draw(){
    DrawCircle(x,y,radius,YELLOW); //inbuilt raylib function which draws a circle /ball:
 }
 ...
 class CpuPaddle:public Paddle{
  private:
          bool check;
  public:
        CpuPaddle(bool c):check(c){}
        CpuPaddle(){}
        void Update(int ball_y){
        if(check)
        {
          if(IsKeyDown(KEY_W)){  //for middle of the paddle lower than the y_ball, paddle up
            y=y-speed;
          }
          if(IsKeyDown(KEY_S)) //for middle of the paddle higher than the y_ball, paddle down.
          {
            y=y+speed;
          }
          limitMovement();
 ...
 int ping(bool check){
    ...
    InitAudioDevice();
    Music theme=LoadMusicStream("music/ping_theme.mp3");
